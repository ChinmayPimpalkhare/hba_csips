...
function log_to_csv(partition::Partition, filename::String)
...
const LOG_DF = DataFrame(d_x=Int[], d_y=Int[], T=Int[], runtime=Float64[], opt_value=Float64[], timestamp=String[])

function log_run_result(d_x::Int, d_y::Int, T::Int, runtime::Float64, opt_value::Float64)
    push!(LOG_DF, (
        d_x=d_x,
        d_y=d_y,
        T=T,
...
function write_log_to_csv(filename::String="EXOTIC_log.csv")
    CSV.write(filename, LOG_DF)
end
function run_single_experiment(d_x::Int, d_y::Int, T::Int, filename)
    low_domain = fill(-1.0, (d_x + 1) * d_y)
    up_domain  = fill(1.0, (d_x + 1) * d_y)
    partition = Partition(low_domain, up_domain, d_x, d_y)
    timing = @timed run_algorithm_StroquOOL!(partition, T)
    node_stack = timing.value
    runtime = timing.time

    best_node = plot_rewards(node_stack)
    opt_value = best_node.reward
    log_to_csv(partition, filename)
    
    log_run_result(d_x, d_y, T, runtime, opt_value)
end


#// [x] Run Parameter Sweep
d_x_list = [5]
d_y_list = [5]
t_list = [100]

for d_x in d_x_list, d_y in d_y_list, t in t_list 
    run_single_experiment(d_x, d_y, t, "dx5_dy5_T100_nodes.csv")
end

#write_log_to_csv("EXOTIC_results_dx_30_dy_30_T_10000.csv")
